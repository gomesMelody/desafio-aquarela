...
from pyspark.sql.functions import col, to_date, max as spark_max, when

spark = SparkSession.builder.appName("hotStreaks").getOrCreate()
...
def hot_streaks():
    ...
    spark_df = spark_df.withColumn('TX_DRYB_10', col('TX_DRYB_10').cast('float'))
    spark_df = spark_df.withColumn('T_DRYB_10', col('T_DRYB_10').cast('float'))
    spark_df = spark_df.withColumn('TX_DRYB_10',
                                   when(col('TX_DRYB_10').isNull(), col('T_DRYB_10')).otherwise(col('TX_DRYB_10')))

    grouped_df = spark_df.groupBy('DATE').agg(spark_max('TX_DRYB_10').alias('TX_DRYB_10')).orderBy('DATE')

    grouped_df = grouped_df.withColumn('TX_DRYB_10', (col('TX_DRYB_10') - 32) * 5.0/9.0)
    ...
    for row in grouped_df:
        ...
        if float(temp) >= start_temperature:
            ...
            if float(temp) >= high_temperature:
                high_temp_days += 1
        else:
            high_temp_days = 0
            current_streak = []
        if len(current_streak) >= min_days and high_temp_days >= min_high_temp_days:
            total_streaks.append(current_streak)
            current_streak = []

    print(f'Total hot streaks: {len(total_streaks)}')
