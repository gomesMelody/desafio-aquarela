...
def hot_streaks():
    ...
    combined_df['TX_DRYB_10'] = pd.to_numeric(combined_df['TX_DRYB_10'], errors='coerce')
    combined_df['T_DRYB_10'] = pd.to_numeric(combined_df['T_DRYB_10'], errors='coerce')
    combined_df.loc[combined_df['TX_DRYB_10'] == 'nan', 'TX_DRYB_10'] = combined_df['T_DRYB_10']

    combined_df = combined_df.sort_values(by='DATE')
    grouped_df = combined_df.groupby(combined_df['DATE'])['TX_DRYB_10'].max().reset_index()

    grouped_df['TX_DRYB_10'] = (grouped_df['TX_DRYB_10'] - 32) * 5.0/9.0
    ...
    for i in range(len(grouped_df)):
        temp = grouped_df.iloc[i]['TX_DRYB_10']
